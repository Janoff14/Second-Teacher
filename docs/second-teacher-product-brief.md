...
> Same content as `_bmad-output/planning-artifacts/second-teacher-product-brief.md`. Use this copy from the `docs/` folder if Cursor fails to open the planning-artifacts path (open the **Second-Teacher** folder in Cursor, or use **File → Open File** with the full Windows path including the drive letter, e.g. `C:\Users\sanja\Second-Teacher\...`).
...
## Next BMAD Step
PRD draft: `docs/prd.md` (mirror of `_bmad-output/planning-artifacts/prd.md`). Next: `[CA] Create Architecture` — `bmad-create-architecture`.
